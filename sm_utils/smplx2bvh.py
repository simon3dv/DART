...
import smplx
import time
from sm_utils import bvh_endpoint, quat


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--model_path", type=str, default="/nvme/data/all_smpl_model/models")
    ...
    parser.add_argument("--poses", type=str, default="/home/simon3dv/code/MotionCraft/samples/humanml3d/t2m_no_face_loss_l4_latentdim128_ffsize512/res_Wave_hello_with_a_cheerful_smile_90.npz")
    parser.add_argument("--fps", type=int, default=60)
    parser.add_argument("--output", type=str, default="/home/simon3dv/code/MotionCraft/samples/humanml3d/t2m_no_face_loss_l4_latentdim128_ffsize512/res_Wave_hello_with_a_cheerful_smile_90.bvh")
...
def smplx2bvh(model_path:str, poses:str, output:str, mirror:bool,
             model_type="smplx", gender="NETURAL",
             num_betas=10, fps=30) -> None:
   ...
   #     "RightThumb",
    ...
    Note, it's order is body22+jaw_leye_reye3+hand30, different from smplx pose rotation order body22+hand30+jaw_leye_reye3, so we need to reorder the rots
    """
    print(f"==debug==> names:{len(names)}")
    # I prepared smpl models only, 
    # but I will release for smplx models recently.
    model = smplx.create(model_path=model_path, 
                        model_type=model_type,
                        gender=gender, 
                        batch_size=1, flat_hand_mean=True) # Note: It's import to set flat_hand_mean=True, to make easier retarget. 
    ...
    if isinstance(poses, str):
        ...
        if poses.endswith(".npz"):
            poses = np.load(poses)
            rots = poses["poses"].reshape(-1, 55, 3)#np.squeeze(poses["poses"], axis=0) # (N, 24, 3)
            trans = poses["trans"].reshape(-1, 3)#np.squeeze(poses["trans"], axis=0) # (N, 3)
            ...
            poses = np.load(poses)
            assert poses.shape[-1] == 578

            rots = poses[:, :55*3].reshape(-1, 55, 3)
            trans = poses[:, 55*3:55*3+3]
            if scaling is not None:
                trans /= scaling
        else:
            print(f"Error! poses={poses}")        
    else:
        if poses.shape[-1] == 578:
            rots = poses[:, :55*3].reshape(-1, 55, 3)
            trans = poses[:, 55*3:55*3+3]
            if scaling is not None:
                trans /= scaling
        else:
            print(f"Error with poses.shape:{poses.shape}")
    ...
    #Note, it's order(joint_names) is body22+jaw_leye_reye3+hand30, different from smplx pose rotation order body22+hand30+jaw_leye_reye3, so we need to reorder the rots
    #rotations_body22, rotations_hands,rotations_jaw_leye_reye = rotations[:,:22],rotations[:,22:52],rotations[:,52:]
    #rotations = np.concatenate([rotations_body22, rotations_jaw_leye_reye, rotations_hands], 1)
    ...
    print(f"==debug2==>smplx2bvh cost {time.time()-tic}s")
    if not output.endswith(".bvh"):
        output = output + ".bvh"
    
    bvh_endpoint.save(output, bvh_data)
     
    #if mirror: # Not Implement for this smplx yet
    ...
    #    bvh_endpoint.save(output_mirror, bvh_data)
