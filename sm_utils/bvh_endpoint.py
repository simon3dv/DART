...
def save_joint(f, data, t, i, save_order, order='zyx', save_positions=False):
    ...
    if end_site:
        ...
        f.write("%s{\n" % t)
        t += '\t'
        prev_point = data['positions'][0,save_order[-2]]
        cur_point = data['positions'][0,i]
        
        if i == 10 or i == 11: # foot:
            prev_point = data['positions'][0,i-3]
            cur_point = data['positions'][0,i]                
            x = cur_point + (cur_point - prev_point) * 0.3 #* 0.3
            f.write("%sOFFSET %f %f %f\n" % (t, x[0], x[1], x[2]))
        elif i==22: #jaw
            f.write("%sOFFSET %f %f %f\n" % (t, 0.0, 0.05, 0.0))            
        elif i==23: #left eye
            prev_point = data['positions'][0,22]
            cur_point = data['positions'][0,15]
            x = cur_point + (cur_point - prev_point) * 0.1            
            f.write("%sOFFSET %f %f %f\n" % (t, x[0], x[1], x[2]))
            ...
            x = cur_point + (cur_point - prev_point) * 1.0
            f.write("%sOFFSET %f %f %f\n" % (t, x[0], x[1], x[2]))            
...
def save(filename, data, save_positions=False, add_first_restpose=False):
