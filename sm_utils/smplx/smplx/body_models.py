...
from .lbs import (
    lbs, vertices2landmarks, find_dynamic_lmk_idx_and_bcoords, lbs_v2)
...
#
#         # Load the model
#         if cfg.modify_root_joint:
#             smplx_path = '/home/linjing/code/Hand4Whole_RELEASE-Fitting/common/human_model_files/smplx/SMPLX_NEUTRAL_NEW.npy'
#             model_data = np.load(smplx_path, allow_pickle=True).item()
#         else:
#             if osp.isdir(model_path):
...
#         # print(full_pose.shape, self.pose_mean.shape)
#         full_pose += self.pose_mean
#
#         batch_size = max(betas.shape[0], global_orient.shape[0],
#                          body_pose.shape[0])
...
class SMPLX(SMPLH):
    ...
    corrective blend shapes, has N=10475 vertices and K=54 joints,
    which includes joints for the neck, jaw, eyeballs and fingers.
    Compared with original SMPLX model, we have face shape parameter (size=100) and expression parameter (size=50)
    '''
    ...
    def __init__(
        ...
        **kwargs
        ...
        if cfg.use_flame and not cfg.modify_root_joint:
            smplx_path = './body_models/human_model_files/smplx/SMPLX_NEUTRAL_WiFlame.npy'
            model_data = np.load(smplx_path, allow_pickle=True).item()
        elif cfg.use_flame and cfg.modify_root_joint:
            smplx_path = '.body_models/human_model_files/smplx/SMPLX_NEUTRAL_NEW_WiFlame.npy'
            model_data = np.load(smplx_path, allow_pickle=True).item()
        elif not cfg.use_flame and cfg.modify_root_joint:
            smplx_path = '.body_models/human_model_files/smplx/SMPLX_NEUTRAL_NEW.npy'
            model_data = np.load(smplx_path, allow_pickle=True).item()
        else:
            if osp.isdir(model_path):
                model_fn = 'SMPLX_{}.{ext}'.format(gender.upper(), ext=ext)
        ...
        self.register_buffer(
            'expr_dirs', to_tensor(to_np(expr_dirs), dtype=dtype))
        if cfg.use_flame:
            print('Use FLAME shape and expression parameters.')
            flame_shapedirs = data_struct.flame_shapedirs
            self.register_buffer(
                'flame_shapedirs', to_tensor(to_np(flame_shapedirs), dtype=dtype))
            ...
            flame_default_shape = torch.zeros([1, 300 - cfg.flame_shape_params], dtype=self.dtype, requires_grad=False)
            self.register_parameter('flame_shape_betas', nn.Parameter(flame_default_shape, requires_grad=False))
            ...
            default_exp = torch.zeros([1, 100 - cfg.flame_expression_params], dtype=self.dtype, requires_grad=False)
            self.register_parameter('flame_expression_betas', nn.Parameter(default_exp, requires_grad=False))
            self.head_idxs = np.load(cfg.face_corr_fname)
    ...
    def forward(
        ...
        **kwargs
        ...
        if cfg.use_flame:
            flame_shape_betas = self.flame_shape_betas.expand(batch_size, -1)
            flame_expression_betas = self.flame_expression_betas.expand(batch_size, -1)
            flame_betas = torch.cat([flame_betas, flame_shape_betas, flame_expression, flame_expression_betas], dim=1)
            flame_shapedirs = self.flame_shapedirs
            vertices, joints = lbs_v2(shape_components, full_pose, self.v_template,
                                   shapedirs, self.posedirs, self.J_regressor, self.parents,
                                   self.lbs_weights, flame_betas, flame_shapedirs, self.head_idxs, pose2rot=pose2rot,
                                   )
        else:
            vertices, joints = lbs(shape_components, full_pose, self.v_template,
                                   shapedirs, self.posedirs,
...
def create(
    ...
    **kwargs
    ...
    if model_type.lower() == 'smpl':
        ...
        raise ValueError(f'Unknown model type {model_type}, exiting! model_path={model_path}')
